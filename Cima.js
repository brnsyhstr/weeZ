...
await cima.sendMessage(m.chat, {text: `${teks}`, contextInfo: {mentionedJid: mention, externalAdReply: {thumbnail: await global.imgreply, title: "© KenzMods Version - 0.1", body: `Selamat ${ucapan()}`, 
...
switch (command) {
case "menu": case "mdjen": case "pjday": case "listpayndment": {
...
Perkenalkan Saya Adalah *KenzMods Assistant* By @${owner} Yang Siap Membantu Anda
...
break
case 'tiktok':{
if (!q) return CimaReply( `Example : ${prefix + command} link`)
if (!q.includes('tiktok')) return CimaReply(`Link Invalid!!`)
require('./lib/tiktok').Tiktok(q).then( data => {
cima.sendMessage(m.chat, { caption: `Here you go!`, video: { url: data.watermark }}, {quoted:m})
})
}
break
case "tiktojjk": case "tt": {
if (!text) return CimaReply(example('linknya'))
...
break
case "alltrxon": case "alltrx": case "alltrxnih": case "listpayment": {
if (!isOwner) return CimaReply(msg.owner)
let imgsc = await prepareWAMessageMedia({ image: await global.imgslide }, { upload: cima.waUploadToServer })
let imgqr = await prepareWAMessageMedia({ image: await global.qris}, { upload: cima.waUploadToServer })
const msgii = await generateWAMessageFromContent(m.chat, {
...
cards: [{
header: proto.Message.InteractiveMessage.Header.fromObject({
title: `*\`Nokos Wa Indonesia +62 🇲🇨\`*
*1 Nokos Harga 5.000 IDR*
*2 Nokos Harga 9.000 IDR*
*3 Nokos Harga 13.000 IDR*
*4 Nokos Harga 16.000 IDR*
*5 Nokos Harga 19.000 IDR*

*\`Nokos Wa Thailand +66🇹🇭\`*
*1 Nokos Harga 6.000 IDR*
*2 Nokos Harga 12.000 IDR* 
*3 Nokos Harga 15.000 IDR*

*\`Nokos Wa Malaysia +60 🇲🇾\`*
*1 Nokos Harga 10.000 IDR*
*2 Nokos Harga 18.000 IDR*
*3 Nokos Harga 26.000 IDR*`, 
hasMediaAttachment: true,
...imgsc
}),
nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
...
UNL I- 13000 IDR
    ...
    [ʙᴇɴᴇғɪᴛ]✳️
ᴀɴᴛɪ ʟᴀɢ
ᴅɪ ᴊᴀᴍɪɴ 𝟷𝟶𝟶% ᴛᴀɴɢɢᴜɴɢ ᴊᴀᴡᴀʙ
ᴡᴇʙ ᴄʟᴏsᴇ ʙᴏᴛ ᴛᴇᴛᴇᴘ ɴʏᴀʟᴀ
ʀᴀᴍ𝟷𝟼 ᴄᴏʀᴇ 𝟺
ᴀɴᴛɪ ʀᴜsᴜʜ 
ɴᴏ ʟᴀɢ
ʜᴇᴍᴀᴛ ᴋᴜᴏᴛᴀ`, 
...
break
case "proses": case "prses": 
case "pross": {
let teks = `*\`DANA RESMI MASUK*✅📦
*ALL TRX NO REFF\`*🙏

* MOHON MENUNGGU OWNER
* ORANG SABAR DI SAYANG TUHAN
* TRX ANDA SEDANG DI PROSES`
cima.relayMessage(m.chat,  {requestPaymentMessage: {currencyCodeIso4217: 'IDR', amount1000: 1000000, requestFrom: m.sender, noteMessage: { extendedTextMessage: { text: teks, contextInfo: { externalAdReply: { showAdAttribution: true}}}}}}, {})
}
break
...
break
case "sendpayment": case "qrispay": case "pay": case "listpayment": {
if (!isOwner) return CimaReply(msg.owner)
let imgsc = await prepareWAMessageMedia({ image: await global.imgslide }, { upload: cima.waUploadToServer })
let imgqr = await prepareWAMessageMedia({ image: await global.qris}, { upload: cima.waUploadToServer })
const msgii = await generateWAMessageFromContent(m.chat, {
...
📜Note: Buyer Harus Kirim Bukti Tf Biar Gaada Hal Yg Tidak Di Inginkan`,
hasMediaAttachment: true,
...imgqr
}),
nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
